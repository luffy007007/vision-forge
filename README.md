...
![Classical CV operations run on one sample image](assets/opencv_grid.png)
...
Twenty-three objects were found in total. Runtimes are from a CPU-only machine and include no warm-up — SAM dominates, which is exactly what the per-stage breakdown exists to show.
...
- No screenshots of the app in this README yet — `assets/opencv_grid.png` above is generated output, not a UI capture.
...
No licence file yet. Note that Ultralytics YOLO is AGPL-3.0, which constrains how the detection module can be redistributed — worth resolving before adding one.
